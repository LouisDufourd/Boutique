...
object JsonItemStack {
    ...
    fun toJson(itemStack: ItemStack): String {
        ...
        if (itemStack.hasItemMeta()) {
            ...
            if (meta.hasLore()) {
                val lore = JsonArray()
                meta.lore!!.forEach(Consumer { str: String? -> lore.add(JsonPrimitive(str)) })
            ...
            if (meta is SkullMeta) {
                ...
                if (bannerMeta.numberOfPatterns() > 0) {
                    ...
                    bannerMeta.patterns
                        ...
                        .forEach { str: String? -> patterns.add(JsonPrimitive(str)) }
                ...
                if (bmeta.hasAuthor() || bmeta.hasPages() || bmeta.hasTitle()) {
                    ...
                    if (bmeta.hasPages()) {
                        val pages = JsonArray()
                        bmeta.pages.forEach(Consumer { str: String? -> pages.add(JsonPrimitive(str)) })
    ...
    fun fromJson(string: String?): ItemStack? {
        ...
        return if (element.isJsonObject) {
            ...
            if (typeElement.isJsonPrimitive) {
                ...
                val data = dataElement?.asInt ?: 0
                val amount = amountElement?.asInt ?: 1
                val material = Material.getMaterial(type)
                if (material == null) {
                    System.err.println("material is null : $material")
                } else {
                    println("material is not null : ${material.name}")
                }
                ...
                if (itemMetaElement != null && itemMetaElement.isJsonObject) {
                        ...
                        jarray.forEach(Consumer { jsonElement: JsonElement ->
                            if (jsonElement.isJsonPrimitive) {
                                ...
                                if (enchantString.contains(":")) {
                                    try {
                                        val splitEnchant =
                                            enchantString.split(":".toRegex()).dropLastWhile { it.isEmpty() }
                                                .toTypedArray()
                    ...
                    if (flagsElement != null && flagsElement.isJsonArray) {
                        ...
                        jarray.forEach(Consumer<JsonElement> { jsonElement: JsonElement ->
                            if (jsonElement.isJsonPrimitive) {
                                for (flag: ItemFlag in ItemFlag.values()) {
                    ...
                    for (clazz: String in BYPASS_CLASS) {
                        if (meta!!.javaClass.simpleName == clazz) {
                            return itemStack
                        }
                    }
                        ...
                        try {
                            ...
                            if (meta is SkullMeta) {
                                ...
                                if (ownerElement != null && ownerElement.isJsonPrimitive) {
                                    meta.setOwner(ownerElement.asString)
                                }
                            } else if (meta is BannerMeta) {
                                ...
                                if (patternsElement != null && patternsElement.isJsonArray) {
                                    ...
                                    jarray.forEach(Consumer { jsonElement: JsonElement ->
                                        ...
                                        if (patternString.contains(":")) {
                                            val splitPattern =
                                                patternString.split(":".toRegex()).dropLastWhile { it.isEmpty() }
                                                    .toTypedArray()
                                            val color = Arrays.stream(DyeColor.values())
                                                .filter({ dyeColor: DyeColor ->
                                                    dyeColor.name.equals(
                                                ...
                                                .findFirst()
                                            val patternType = PatternType.getByIdentifier(splitPattern[1])
                                ...
                                if (storedEnchantsElement != null && storedEnchantsElement.isJsonArray) {
                                    ...
                                    jarray.forEach(Consumer { jsonElement: JsonElement ->
                                        if (jsonElement.isJsonPrimitive) {
                                            ...
                                            if (enchantString.contains(":")) {
                                                try {
                                                    val splitEnchant = enchantString.split(":".toRegex())
                                ...
                                if (titleElement != null && titleElement.isJsonPrimitive) {
                                    bmeta.setTitle(titleElement.asString)
                                }
                                ...
                                if (customEffectsElement != null && customEffectsElement.isJsonArray) {
                                    ...
                                    jarray.forEach(Consumer { jsonElement: JsonElement ->
                                        if (jsonElement.isJsonPrimitive) {
                                            ...
                                            if (enchantString.contains(":")) {
                                                try {
                                                    val splitPotions = enchantString.split(":".toRegex())
                                                        .dropLastWhile { it.isEmpty() }
                                                        .toTypedArray()
                                                    val potionType = PotionEffectType.getByName(splitPotions[0])
                                ...
                                if (effectTypeElement != null && effectTypeElement.isJsonPrimitive) {
                                    val effectType = FireworkEffect.Type.valueOf(effectTypeElement.asString)
                                    if (effectType != null) {
                                ...
                                if (effectArrayElement != null && effectArrayElement.isJsonArray) {
                                    effectArrayElement.asJsonArray.forEach(Consumer { jsonElement: JsonElement ->
                                        if (jsonElement.isJsonObject) {
                                            ...
                                            val fadeColorsElement = jsonObject["fade-colors"]
                                            if (effectTypeElement != null && effectTypeElement.isJsonPrimitive) {
                                                val effectType: FireworkEffect.Type =
                                                    FireworkEffect.Type.valueOf(effectTypeElement.asString)
                                                if (effectType != null) {
                                                    val colors: MutableList<Color?> = ArrayList()
                                                    if (colorsElement != null && colorsElement.isJsonArray) colorsElement.asJsonArray.forEach(
                                                        Consumer { colorElement: JsonElement ->
                ...
                val im = itemStack.itemMeta as Damageable?
                im!!.damage = data
